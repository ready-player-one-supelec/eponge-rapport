...
\begin{equation}
 C = -\frac{1}{n}\sum_x[y\ln(a) + (1-y)\ln(1-a)]\; \text{, \parbox[t]{15em}{$n$ la taille du batch\\ $x$ les exemples du batch\\
...
\begin{align}
 \frac{\partial C}{\partial \omega} &= (a-y)\sigma'(z)x \text{, \parbox[t]{15em}{où $z$ est l'antécédant de $a$ par la fonction d'activation}}\\
 \frac{\partial C}{\partial b} &= (a-y)\sigma'(z) 
\end{align}

En observant la fonction sigmoide figure \ref{fig:sigmoid_function}, on se rend compte que le problème provient de $\sigma'(z)$.
...
partie droite de la courbe, où la pente est très faible. Cela provient du fait que $\sigma'(z) = a(1-a)$.
...
\begin{align}
  ...
  \frac{\partial C}{\partial \omega} &= (a-y)x \\
...
\begin{equation}
 \frac{\partial C}{\partial b} = \frac{\partial C}{\partial a} \frac{\partial a}{\partial b} = \frac{\partial C}{\partial a} a(1-a)
\end{equation}
...
La formule de l'entropie d'une variable aléatoire $X$ est 
\begin{equation}
...
l'entropie est maximale. On a alors que le nombre de bits nécessaires pour définir le symbole avec certitude est $\log_2(N)$ où $N$ est le nombre
...
L'entropie croisée $-q\log(p)$ est, en mathématiques, une mesure de la distance entre deux distributions. On peut là encore y retrouver une interprétation
...
l'entropie croisée $-q\log(p)$ mesure à quel point ce même système est adapté pour chiffrer un texte où chaque lettre apparaît selon la distribution $p$.
...
on a une couche de plusieurs neurones) et d'une fonction d'activation sigmoide donnant une sortie $q$, alors

\begin{equation}
 \mathbb{P}(X=x) = \begin{cases}q &\text{ si $x=1$}\\1-q &\text{ si $x=0$}\end{cases} \text{, \parbox[t]{15em}{où $X$ représente notre réseau\\
 et $x$ représente les observations}}
 \label{eq:proba}
\end{equation}
...
\begin{equation}
 \label{eq:ddp}
 \mathbb{P}(X=x) = q^x(1-q)^{1-x}
\end{equation}

Ainsi sur un jeu de donnée $\{x_1, x_2, ..., x_N\}$, l'équation \ref{eq:vraisemblance} présente la vraisemblance de notre loi, en posant

\begin{equation}
 p = \frac{\sum_{i=1}^N x_i}{N} \text{, la distribution empirique observée}
\end{equation}


\begin{align*}
 \mathbb{L}(x_1,x_2,...,x_N,q) &= q^{\sum_{i=1}^N x_i}(1-q)^{\sum_{i=1}^N (1-x_i)}\\
 &= q^{Np}(1-q)^{N(1-p)}  
...
\begin{equation}
 \frac{1}{N}\log(\mathbb{L}(x_1,x_2,...,x_N,q)) = p\ln(q) + (1-p)\ln(1-q) 
 \label{eq:log-vraisemblance}
\end{equation}

D'après \ref{eq:log-vraisemblance}, la log-vraisemblance de notre système est l'opposé de l'entropie croisée. Ainsi, on comprend que minimiser 
l'entropie croisée revient à utiliser un estimateur de maximum de vraisemblance. Notre système est donc d'autant plus performant que son 
entropie croisée est faible.
